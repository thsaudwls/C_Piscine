...
/*   Updated: 2022/09/07 20:10:18 by rnyeong._.j      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int key_press(int key, t_opt *game)
{
	...
	mlx_string_put(game->mlx, game->win, 65, 10, 0xFFFFFF, ft_itoa(game->step_num));
    return (0);
}
...
void    move_up(t_opt *game)
{
    ...
    if (game->perfect_map[i - game->wid] == 'C')
        game->get ++;
    if (game->perfect_map[i - game->wid] != '1' && game->perfect_map[i - game->wid] != 'E')
    {
        game->step_num ++;
